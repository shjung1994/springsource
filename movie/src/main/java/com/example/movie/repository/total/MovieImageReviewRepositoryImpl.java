...
public class MovieImageReviewRepositoryImpl extends QuerydslRepositorySupport implements MovieImageReviewRepository {
    ...
    public Page<Object[]> getTotalList(String type, String keyword, Pageable pageable) {
        ...
        // from 절 전까지
        JPQLQuery<Long> count = JPAExpressions.select(review.countDistinct()).from(review).where(review.movie.eq(movieImage.movie));
        JPQLQuery<Double> avg = JPAExpressions.select(review.grade.avg().round()).from(review).where(review.movie.eq(movieImage.movie));
        ...
        // sort 기준이 여러개 일 수 있어서 foreach로 돌림
        sort.stream().forEach(order -> {
            // querydsl.core.types.Order
